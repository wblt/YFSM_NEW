...
#define FTDefaultTintColor          [UIColor colorWithWhite:1.0 alpha:0.9]          //[UIColor colorWithRed:80/255.f green:80/255.f blue:80/255.f alpha:1.f]
#define FTDefaultTextColor          [UIColor                    whiteColor]
#define FTDefaultMenuFont           [UIFont                     systemFontOfSize:14]
#define FTDefaultMenuWidth                                      149.0               //120.0
#define FTDefaultMenuIconWidth                                  20.0
#define FTDefaultMenuRowHeight                                  38.0                //40.0
#define FTDefaultMenuArrowHeight                                10.0
#define FTDefaultMenuArrowWidth                                 7.0
#define FTDefaultMenuCornerRadius                               4.0
#define FTDefaultMargin                                         8.0                 //4.0
...
-(instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier menuName:(NSString *)menuName iconImageName:(NSString *)iconImageName
{
    ...
    if (self) {
        ...
        if (iconImage) {
            ...
            menuNameRect = CGRectMake(margin, 5, FTDefaultMenuWidth - margin*2, FTDefaultMenuRowHeight - 10);
        }
        ...
        _menuNameLabel.layer.cornerRadius = 2;
        _menuNameLabel.clipsToBounds      = YES;
...
-(void)setSelected:(BOOL)selected animated:(BOOL)animated {
    ...
    if (selected) {
        ...
        _menuNameLabel.backgroundColor = [UIColor colorWithRed:32/255.f green:146/255.f blue:224/255.f alpha:1.f];
    }else{
        _menuNameLabel.textColor       = [UIColor colorWithRed:123/255.f green:124/255.f blue:124/255.f alpha:1.f];
        _menuNameLabel.backgroundColor = [UIColor colorWithWhite:1.0 alpha:0.0];
    }
}
...
@property (nonatomic, assign) BOOL                        isF;//第一次默认选择最后一行
@property (nonatomic, assign) NSInteger                   currentSelect;
@property (nonatomic, assign) NSInteger                   currentRow;//最后一次选择
                ...
                doneBlock:(FTPopOverMenuDoneBlock)doneBlock
{
    ...
    [_menuTableView selectRowAtIndexPath:[NSIndexPath indexPathForRow:index-1 inSection:0] animated:NO scrollPosition:(UITableViewScrollPositionTop)];
    _menuTableView.scrollEnabled = shouldAutoScroll;
    
    switch (_arrowDirection) {
        case FTPopOverMenuArrowDirectionUp:
        _menuTableView.frame = CGRectMake(0, FTDefaultMenuArrowHeight + FTDefaultMenuCornerRadius, self.frame.size.width, 186);
        break;
...
{
    [[self sharedInstance] showForSender:sender senderFrame:CGRectNull withMenu:menuArray withSelect:0 imageNameArray:nil doneBlock:doneBlock dismissBlock:dismissBlock];
}
...
+ (void) showForSender:(UIView *)sender
            ...
            withSelect:(NSInteger)index
             doneBlock:(FTPopOverMenuDoneBlock)doneBlock
          dismissBlock:(FTPopOverMenuDismissBlock)dismissBlock {
    [[self sharedInstance] showForSender:sender senderFrame:CGRectNull withMenu:menuArray withSelect:index imageNameArray:nil doneBlock:doneBlock dismissBlock:dismissBlock];
}
...
+ (void) showForSender:(UIView *)sender
        ...
        imageNameArray:(NSArray<NSString*> *)imageNameArray
          ...
          dismissBlock:(FTPopOverMenuDismissBlock)dismissBlock;
{
    [[self sharedInstance] showForSender:sender senderFrame:CGRectNull withMenu:menuArray withSelect:0 imageNameArray:imageNameArray doneBlock:doneBlock dismissBlock:dismissBlock];
}
...
+ (void) showFromEvent:(UIEvent *)event
          ...
          dismissBlock:(FTPopOverMenuDismissBlock)dismissBlock
{
    [[self sharedInstance] showForSender:[event.allTouches.anyObject view] senderFrame:CGRectNull withMenu:menuArray withSelect:0 imageNameArray:nil doneBlock:doneBlock dismissBlock:dismissBlock];
}
...
+ (void) showFromEvent:(UIEvent *)event
        ...
        imageNameArray:(NSArray<NSString*> *)imageNameArray
          ...
          dismissBlock:(FTPopOverMenuDismissBlock)dismissBlock
{
    [[self sharedInstance] showForSender:[event.allTouches.anyObject view] senderFrame:CGRectNull withMenu:menuArray withSelect:0 imageNameArray:imageNameArray doneBlock:doneBlock dismissBlock:dismissBlock];
...
+ (void) showFromSenderFrame:(CGRect )senderFrame
                ...
                dismissBlock:(FTPopOverMenuDismissBlock)dismissBlock
{
    [[self sharedInstance] showForSender:nil senderFrame:senderFrame withMenu:menuArray withSelect:0 imageNameArray:nil doneBlock:doneBlock dismissBlock:dismissBlock];
}
+ (void) showFromSenderFrame:(CGRect )senderFrame
                    withMenu:(NSArray<NSString*> *)menuArray
              imageNameArray:(NSArray<NSString*> *)imageNameArray
                ...
                dismissBlock:(FTPopOverMenuDismissBlock)dismissBlock
{
    [[self sharedInstance] showForSender:nil senderFrame:senderFrame withMenu:menuArray withSelect:0 imageNameArray:imageNameArray doneBlock:doneBlock dismissBlock:dismissBlock];
}
...
-(void)adjustPopOverMenu
{
    ...
    if (arrowDirection == FTPopOverMenuArrowDirectionUp) {
        menuRect = CGRectMake(menuX, (senderRect.origin.y + senderRect.size.height + FTDefaultMargin), FTDefaultMenuWidth, 186 + FTDefaultMenuArrowHeight + FTDefaultMenuCornerRadius);
